...
resource "aws_instance" "http-rdir" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "tmux new -d \"sudo socat TCP4-LISTEN:80,fork TCP4:${element(var.redirect_to, count.index)}:80\" ';' split \"sudo socat TCP4-LISTEN:443,fork TCP4:${element(var.redirect_to, count.index)}:443\" ';' split \"sudo socat TCP4-LISTEN:7443,fork TCP4:${element(var.redirect_to, count.index)}:7443\""
    ]
